...
final class AuthManager: ObservableObject {
    ...
    private func request<T: Encodable>(
        ...
        if !(200...299).contains(http.statusCode) {
            ...
            // 1) Print raw body in Xcode console Debugging
...
extension AuthManager {
    ...
    /// Calls the delete_current_user() Postgres RPC (SECURITY DEFINER) which
    /// deletes the caller from auth.users without needing a service role key.
    ///
    /// One-time SQL setup in Supabase SQL Editor:
    ///
    ...
    func deleteAccount() async throws {
        ...
        // Step 1: delete user data from public tables (user_preferences etc.)
        try? await deleteUserData(uid: uid, token: token)

        // Step 2: call the SECURITY DEFINER RPC to delete the auth.users row.
        // The function runs as postgres (bypasses RLS) so no service role key needed.
        let url = supabaseURL.appendingPathComponent("rest/v1/rpc/delete_current_user")
        ...
        req.setValue("application/json", forHTTPHeaderField: "Content-Type")
        req.httpBody = Data("{}".utf8) // RPC requires a JSON body even when there are no params
        ...
        if !(200...299).contains(http.statusCode) {
            ...
            throw NSError(domain: "Supabase", code: http.statusCode,
                          userInfo: [NSLocalizedDescriptionKey: "Delete failed (HTTP \(http.statusCode)): \(raw)"])
        }
